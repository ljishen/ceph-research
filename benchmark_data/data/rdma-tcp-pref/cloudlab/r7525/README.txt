These are tests between a r7525 node and its local BlueField2 DUP/SmartNIC, and comparing the throughput performance between usnig the TCP and RDMA as the messenger type.


# add the following ceph.conf configuation to vstart.sh (near the line of "osd objectstore = ...")
ms_public_type = async+rdma
ms_async_rdma_device_name = mlx5_3	<-- need to update 
ms_async_rdma_port_num = 1	<-- need to update 
ms_async_rdma_local_gid = 0000:0000:0000:0000:0000:ffff:0a0a:0201	<-- need to update 
...
# and update the ms_async_rdma_device_name, ms_async_rdma_port_num, and ms_async_rdma_local_gid in the ceph.conf to match the client node.
